# AGENTS.md — ACVM Signal

## Product mission

ACVM Signal turns New Zealand MPI ACVM register data into commercial regulatory intelligence for veterinary medicines, agricultural chemicals, and vertebrate toxic agents.

Protect the core value proposition: reliable monitoring of registrations and material changes, searchable product intelligence, competitor/registrant history, and source-backed provenance.

## Current architecture

- Next.js 15 + React 19 + TypeScript
- PostgreSQL via `postgres`
- Railway services: web, Postgres, scheduled sync
- MPI CSV discovery/ingestion and snapshot diffing
- Stripe checkout foundation
- Key scripts: `npm run sync`, `npm run alerts`
- Validation: `npm run typecheck`, `npm run build`

## Non-negotiable product/data rules

- Registration number is the primary normalisation anchor unless the source model requires otherwise.
- Preserve source provenance and raw-source traceability.
- Keep MPI attribution/licensing language accurate.
- ACVM Signal is independent and must not be represented as MPI-endorsed.
- Regulatory intelligence is not a substitute for checking the official source for regulatory decisions.
- Malformed source rows should be isolated/skipped safely rather than breaking an entire import where feasible.
- Preserve idempotence: an unchanged source snapshot must not create false changes/events.
- Never fabricate product registrations, conditions, cancellations, suspensions, labels, dates, registrants, ingredients, or source links.
- Clearly separate official source fields from derived classifications or intelligence.
- Do not expose Stripe secrets or database credentials to the client.
